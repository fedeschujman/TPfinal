...
export default function ComponenteNavbar({handleLogout, user}) {
  return (
    ...
    <Disclosure as="nav" className="bg-gray-800">
      {({ open }) => (
        ...
        {
          user.state?<button onClick={handleLogout} className={classNames(
            false ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white',
            'rounded-md px-3 py-2 text-sm font-medium'
          )}>Cerrar sesión</button>:<Link to='/login' className={classNames(
            false ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white',
            'rounded-md px-3 py-2 text-sm font-medium'
          )}>Iniciar sesión</Link>
        }
